...
# Function to create a binary ellipse edge image with noise, occlusions, and additional shapes and curves
def create_ellipse_with_noise_occlusions(size, center, axes, angle):
    ...
    child_axes_ratio = None
    # Add random noise
    noise = np.random.normal(0, 10, size=image.shape).astype(int)
    image = np.clip(image + noise, 0, 255)

    # # Add random occlusions (black rectangles)
    # for _ in range(np.random.randint(2, 5)):
    #     occlusion_size = np.random.randint(5, 15, size=(2,))
    #     occlusion_position = np.random.randint(0, size - occlusion_size, size=(2,))
    #     image[occlusion_position[1]:occlusion_position[1] + occlusion_size[1],
    #           occlusion_position[0]:occlusion_position[0] + occlusion_size[0]] = 0
    ...
    # # Add tiny random curves as noise
    # for _ in range(np.random.randint(3, 6)):
    #     curve_pts = np.random.randint(0, size, size=(3, 2))
    #     curve_pts = curve_pts.reshape((-1, 1, 2))
    #     cv2.polylines(image, [curve_pts], isClosed=False, color=255, thickness=1)
...
# os.makedirs("BB_labels_1", exist_ok=True)
# os.makedirs("BB_labels_2", exist_ok=True)

# Plot the saved image with grid using matplotlib
for i in range(2501, 2901):
    ...
    noisy_image, _ = create_ellipse_with_noise_occlusions(size, center, axes, angle)
    save_path_image = f"BB/image_{i}.png"
    save_path_outer_label = f"BB_labels_2/outer_bbox_label_{i}.txt"
    save_path_ellipse_label = f"BB_labels_1/presence_label_{i}.txt"
