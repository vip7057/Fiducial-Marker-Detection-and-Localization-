...
def load_and_preprocess_images(image_folder, label_folder, num_images):
    ...
    for i in range(1, num_images + 1):
        ...
        if image_folder == "C:/Users/vipul/Desktop/Programming Project/DATASET\ellipse_absence_images_rgb":
            image_path = os.path.join(image_folder, f"absence_image_{i}.png")
            label_path = os.path.join(label_folder, f"absence_label_{i}.txt")
        elif image_folder == "C:/Users/vipul/Desktop/Programming Project/DATASET\ellipse_presence_images_rgb":
            image_path = os.path.join(image_folder, f"presence_image_{i}.png")
        ...
        #Load 1 label
        with open(label_path, "r") as label_file:
            # Read the first value from the line and convert it to float
...
presence_images, presence_labels = load_and_preprocess_images(
    os.path.join(dataset_folder, "ellipse_presence_images_rgb"),
    os.path.join(dataset_folder, "ellipse_presence_labels"),
    num_images=3712
)
...
print("Dev Images Shape:", dev_images.shape)
...
def f1_score(y_true, y_pred):
    ...
    y_pred = K.cast(y_pred, dtype='float32')  # Cast y_pred to float32
...
model = models.Sequential([
    ...
    layers.Dense(1, activation='sigmoid')  # Output layer with 1 for presence
])
#
# Compile the model with the custom loss function
model.compile(optimizer=Adam(learning_rate=0.001), loss='binary_crossentropy', metrics=['accuracy', f1_score])


# Display the model summary
model.summary()

checkpoint_filepath = 'best_model_0704_1.keras'
...
# Load the saved model on GPU if available
with tf.device('/GPU:0'):  # Specify GPU device
    model = tf.keras.models.load_model('best_model_0704_1.keras', custom_objects={'f1_score': f1_score})

# # Evaluate the model on the test set
test_loss, test_accuracy, test_f1_score = model.evaluate(test_images, test_labels)
